...
@WebServlet("/contents/dramaEdit.do")
public class DramaEditController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		req.getRequestDispatcher("/contents/dramaedit.jsp").forward(req, resp);
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//데이터 베이스에 update 사용
		ContentsDAO dao = new ContentsDAO();
		int result = dao.dramaEdit(dto);
		dao.close();
		
		if(result == 1) {
			PrintWriter write = resp.getWriter();
			String script = "<script>alert('드라마정보 수정이 완료되었습니다.'); "
					+ "location.href='./dramaList.do'</script>";
			write.print(script);
			write.close();
		} else {
			PrintWriter write = resp.getWriter();
