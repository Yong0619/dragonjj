...
public class MemberPaging {
	public static String pagingstr(int totalCount, int pageSize,
		...
		if(pageTemp != 1) {
			pagingStr += "<a href='"+reqUrl+"?pageNum=1&sort="+sort+"'>[처음]</a>";
			pagingStr += "<a href='"+reqUrl+"?pageNum="
					+(pageTemp-1)+"&sort="+sort+"'>[이전]</a>";
		}
		//각 페이지 번호 출력
		int blockcount= 1;
		...
		if(pageTemp <= totalpages) {
			pagingStr += "<a href='"+reqUrl+"?pageNum="+pageTemp+"&sort="+sort+"'>[다음]</a>";
			pagingStr += "<a href='"+reqUrl+"?pageNum="+totalpages+"&sort="+sort+"'>[마지막]</a>";
		}
		return pagingStr;
	}
}
