...
<div class="hdlist_hd">
	...
	<div class="sort">
		<form method="post" action="dramaList.do">
...
<c:choose>
			...
			<c:forEach items="${ drlists }" var="drlists" varStatus="loop">
				<tr id="hddetail">
					<td id="btn"><button type="button"
							onclick="location.href='/project01/contents/dramaEpisodeEdit.do?title=${ drlists.title }'">수정하기</button></td>
					<td
						onclick="location.href='/project01/dramapage/dramaview.do?idx=${ drlists.idx }&&title=${ drlists.title }'"
						style="cursor: pointer;"><img src="../uploads/${ drlists.sfile }" id="hdmovieimg"></td>
					<td
						onclick="location.href='/project01/dramapage/dramaview.do?idx=${ drlists.idx }&&title=${ drlists.title }'"
						style="cursor: pointer;">${ drlists.title }</td>
					<td
						onclick="location.href='/project01/dramapage/dramaview.do?idx=${ drlists.idx }&&title=${ drlists.title }'"
						style="cursor: pointer;">${ drlists.cast }</td>
					<td
						onclick="location.href='/project01/dramapage/dramaview.do?idx=${ drlists.idx }&&title=${ drlists.title }'"
						style="cursor: pointer;">${ drlists.price }</td>
				</tr>
			</c:forEach>
		</table>
	</c:when>
	...
	<c:otherwise>
		<table id="hdmovietable">
			...
			<c:forEach items="${ drlists }" var="drlists" varStatus="loop">
			<tr
				onclick="location.href='/project01/contents/dramaview.do?idx=${ drlists.idx }&&title=${ drlists.title }'"
				...
				<td><img src="../uploads/${ drlists.sfile }" id="hdmovieimg"></td>
				<td>${ drlists.title }</td>
				<td>${ drlists.cast }</td>
				<td>${ drlists.price }</td>
			</tr>
